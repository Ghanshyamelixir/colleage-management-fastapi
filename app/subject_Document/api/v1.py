...
def sub_doc_create(request: schemas.SubjectDocumentCreate, db: Session = Depends(get_db)):
...
def get_all_sub_doc_data(db: Session = Depends(get_db)):
    alldata = db.query(SubjectDocument).all()
    return alldata


@route.get('/{id}', response_model=schemas.SubjectDataId)
def get_data_using_id(id, db: Session = Depends(get_db)):
...
@route.put('/{id}')
def update_record(id, request: schemas.SubjectDocumentUpdate, db: Session = Depends(get_db)):
