...
def create(request: SemesterBody,db: Session = Depends(get_db)):
...
def show_record(db: Session = Depends(get_db)):
...
def update_sem_record(id, request: schemas.SemesterUpdate, db: Session = Depends(get_db)):
